...
export async function GET(req: Request) {
  ...
  if (error) {
    return NextResponse.redirect(`${APP_URL}/settings?oauth_error=${encodeURIComponent(error)}`)
  }

  if (!code || !state) {
    return NextResponse.redirect(`${APP_URL}/settings?oauth_error=missing_code_or_state`)
  }
  ...
  if (!appClientId || !appClientSecret || !redirectUri) {
    return NextResponse.redirect(`${APP_URL}/settings?oauth_error=missing_env_vars`)
  }
  ...
  if (!tokenRes.ok) {
    ...
    return NextResponse.redirect(
      `${APP_URL}/settings?oauth_error=${encodeURIComponent('token_exchange_failed')}`
    )
  }
  ...
  const { error: dbError } = await supabase.from('oauth_tokens').upsert(upsertData, {
    onConflict: clientId ? 'client_id,provider' : 'provider',
  })

  if (dbError) {
    console.error('Failed to store GHL token:', dbError)
    return NextResponse.redirect(
      `${APP_URL}/settings?oauth_error=${encodeURIComponent('db_store_failed')}`
    )
  }
  ...
  return NextResponse.redirect(`${APP_URL}/settings?oauth_success=true`)
}
